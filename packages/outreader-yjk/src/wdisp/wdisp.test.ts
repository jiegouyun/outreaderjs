...
describe('wdisp', () => {
  ...
  it('should extract driftSeismicX', async () => {
    ...
    expect(driftSeismicX.storeyID[4]).toBe(42);
    expect(driftSeismicX.drift[4]).toBe(921);
  });

  it('should extract driftSeismicTwoWayX', async () => {
    const driftSeismicTwoWayX = wdisp.driftSeismicTwoWayX;
    expect(driftSeismicTwoWayX.storeyID[4]).toBe(42);
    expect(driftSeismicTwoWayX.drift[4]).toBe(909);
  });

  it('should extract driftSeismicXEccP', async () => {
    const driftSeismicXEccP = wdisp.driftSeismicXEccP;
    expect(driftSeismicXEccP.storeyID[4]).toBe(42);
    expect(driftSeismicXEccP.drift[4]).toBe(878);
  });

  it('should extract driftSeismicXEccN', async () => {
    const driftSeismicXEccN = wdisp.driftSeismicXEccN;
    expect(driftSeismicXEccN.storeyID[4]).toBe(42);
    expect(driftSeismicXEccN.drift[4]).toBe(944);
  });

  it('should extract driftSeismicY', async () => {
    const driftSeismicY = wdisp.driftSeismicY;
    expect(driftSeismicY.storeyID[4]).toBe(42);
    expect(driftSeismicY.drift[4]).toBe(832);
  });

  it('should extract driftSeismicTwoWayY', async () => {
    const driftSeismicTwoWayY = wdisp.driftSeismicTwoWayY;
    expect(driftSeismicTwoWayY.storeyID[4]).toBe(42);
    expect(driftSeismicTwoWayY.drift[4]).toBe(818);
  });

  it('should extract driftSeismicYEccP', async () => {
    const driftSeismicYEccP = wdisp.driftSeismicYEccP;
    expect(driftSeismicYEccP.storeyID[4]).toBe(42);
    expect(driftSeismicYEccP.drift[4]).toBe(787);
  });

  it('should extract driftSeismicYEccN', async () => {
    const driftSeismicYEccN = wdisp.driftSeismicYEccN;
    expect(driftSeismicYEccN.storeyID[4]).toBe(42);
    expect(driftSeismicYEccN.drift[4]).toBe(793);
  });

  it('should extract driftWindXP', async () => {
    const driftWindXP = wdisp.driftWindXP;
    expect(driftWindXP.storeyID[4]).toBe(42);
    expect(driftWindXP.drift[4]).toBe(846);
    expect(driftWindXP.ratio[4]).toBe(1.01);
  });

  it('should extract driftCrossWindXN', async () => {
    const driftCrossWindXN = wdisp.driftCrossWindXN;
    expect(driftCrossWindXN.storeyID[4]).toBe(42);
    expect(driftCrossWindXN.drift[4]).toBe(777);
    expect(driftCrossWindXN.ratio[4]).toBe(1.0);
  });

  it('should extract driftWindYP', async () => {
    const driftWindYP = wdisp.driftWindYP;
    expect(driftWindYP.storeyID[4]).toBe(42);
    expect(driftWindYP.drift[4]).toBe(597);
    expect(driftWindYP.ratio[4]).toBe(1.0);
  });

  it('should extract driftCrossWindYN', async () => {
    const driftCrossWindYN = wdisp.driftCrossWindYN;
    expect(driftCrossWindYN.storeyID[4]).toBe(42);
    expect(driftCrossWindYN.drift[4]).toBe(959);
    expect(driftCrossWindYN.ratio[4]).toBe(1.01);
  });

  it('should extract ratioSeismicX', async () => {
    const ratioSeismicX = wdisp.ratioSeismicX;
    expect(ratioSeismicX.storeyID[4]).toBe(42);
    expect(ratioSeismicX.ratioD[4]).toBe(1.01);
  });

  it('should extract ratioSeismicXEccP', async () => {
    const ratioSeismicXEccP = wdisp.ratioSeismicXEccP;
    expect(ratioSeismicXEccP.storeyID[4]).toBe(42);
  ...
  it('should extract ratioSeismicXEccN', async () => {
    ...
    expect(ratioSeismicXEccN.storeyID[4]).toBe(42);
    expect(ratioSeismicXEccN.ratioD[4]).toBe(1.04);
  });

  it('should extract ratioSeismicY', async () => {
    const ratioSeismicY = wdisp.ratioSeismicY;
    expect(ratioSeismicY.storeyID[4]).toBe(42);
    expect(ratioSeismicY.ratioD[4]).toBe(1.0);
  });

  it('should extract ratioSeismicYEccP', async () => {
    const ratioSeismicYEccP = wdisp.ratioSeismicYEccP;
    expect(ratioSeismicYEccP.storeyID[4]).toBe(42);
    expect(ratioSeismicYEccP.ratioD[4]).toBe(1.06);
  });

  it('should extract ratioSeismicYEccN', async () => {
    const ratioSeismicYEccN = wdisp.ratioSeismicYEccN;
    expect(ratioSeismicYEccN.storeyID[4]).toBe(42);
    expect(ratioSeismicYEccN.ratioD[4]).toBe(1.06);
  });
});
