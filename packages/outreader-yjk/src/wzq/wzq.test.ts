...
describe('wzq', () => {
  ...
  it('should extract modeCoupling', async () => {
    ...
    expect(modeCoupling.period[3]).toBe(1.8113);
    expect(modeCoupling.factorX[3]).toBe(0.86);
  });

  it('should extract modeSeismic', async () => {
    const modeSeismic = wzq.modeSeismic;
    expect(modeSeismic.modeID[3]).toBe(4);
    expect(modeSeismic.period[3]).toBe(1.8202);
    expect(modeSeismic.factorX[3]).toBe(0.82);
  });

  it('should extract modeMass', async () => {
    const modeMass = wzq.modeMass;
    expect(modeMass.modeID[3]).toBe(4);
    expect(modeMass.factorX[3]).toBe(13.01);
    expect(modeMass.factorY[3]).toBe(0.06);
  });

  it('should extract seismicForce', async () => {
    const seismicForce = wzq.seismicForce;
    expect(seismicForce.storeyID[4]).toBe(42);
    expect(seismicForce.shearX[4]).toBe(2995.98);
    expect(seismicForce.shearY[4]).toBe(3592.35);
  });
});
