...
export async function readWmassOutput(dir: string): Promise<IWmass> {
  ...
  await readLineByLine(file, (line: string) => {
    ...
    if (!wmass.generalInformation.allExtracted) {
      wmass.generalInformation = extractGeneralInformation(lineArray, wmass.generalInformation);
    }

    // Extract calculationControl{}
    if (!wmass.calculationControl.allExtracted) {
      wmass.calculationControl = extractCalculationControl(lineArray, wmass.calculationControl);
    }

    // Extract windInformation{}
    if (!wmass.windInformation.allExtracted) {
      wmass.windInformation = extractWindInformation(lineArray, wmass.windInformation);
    }

    // Extract seismicInformation{}
    if (!wmass.seismicInformation.allExtracted) {
      wmass.seismicInformation = extractSeismicInformation(lineArray, wmass.seismicInformation);
    }
    ...
    if (!wmass.constraintFloorStiffnessRatio.allExtracted) {
      wmass.constraintFloorStiffnessRatio = extractConstraintFloorStiffnessRatio(lineArray, wmass.constraintFloorStiffnessRatio);
    }
...
export function extractGeneralInformation(
  ...
  if (!Boolean(generalInformation.structuralMaterial)) {
    generalInformation.structuralMaterial = extractData(lineArray, "结构材料信息", 0, 1);
  }
  ...
  if (!Boolean(generalInformation.basement)) {
    generalInformation.basement = Number(extractData(lineArray, "地下室层数", 0, 1));
  }
  if (!Boolean(generalInformation.constraintFloor)) {
    generalInformation.constraintFloor = Number( extractData(lineArray, "嵌固端所在层号(层顶嵌固)", 0, 1));
  }
  if (!Boolean(generalInformation.podium)) {
    generalInformation.podium = Number( extractData(lineArray, "裙房层数", 0, 1));
  }
  if (!Boolean(generalInformation.transferStorey)) {
    generalInformation.transferStorey = Number( extractData(lineArray, "转换层所在层号", 0, 1));
  }
  if (!Boolean(generalInformation.reinforceStorey)) {
    generalInformation.reinforceStorey = Number( extractData(lineArray, "加强层所在层号", 0, 1));
  }

  generalInformation.allExtracted = checkObjectKeysIfAllExtracted(generalInformation);
...
export function extractCalculationControl(
  ...
  if (!Boolean(calculationControl.couplingBeamFactorSeismic)) {
    calculationControl.couplingBeamFactorSeismic = Number( extractData(lineArray, "连梁刚度折减系数(地震)", 0, 1));
  }
  if (!Boolean(calculationControl.couplingBeamFactorWind)) {
    calculationControl.couplingBeamFactorWind = Number( extractData(lineArray, "连梁刚度折减系数(风)", 0, 1));
  }
  if (!Boolean(calculationControl.rigidFloorAssumption)) {
    calculationControl.rigidFloorAssumption = extractData(lineArray, "刚性楼板假定", 0, 1);
  }

  calculationControl.allExtracted = checkObjectKeysIfAllExtracted(calculationControl);
...
export function extractWindInformation(
  ...
  if (!Boolean(windInformation.useAssigned)) {
    windInformation.useAssigned = extractData(lineArray, "使用指定风荷载数据", 0, 1);
  }
  ...
  if (!Boolean(windInformation.terrainRoughness)) {
    windInformation.terrainRoughness = extractData(lineArray, "地面粗糙程度", 0, 1);
  }
  if (!Boolean(windInformation.pressureModified)) {
    windInformation.pressureModified = Number(extractData(lineArray, "修正后的基本风压", 0, 2));
  }
  if (!Boolean(windInformation.dampingRatio)) {
    windInformation.dampingRatio = Number(extractData(lineArray, "风荷载计算用阻尼比", 0, 1));
  }
  if (!Boolean(windInformation.pressureComfort)) {
    windInformation.pressureComfort = Number(extractData(lineArray, "舒适度验算用基本风压", 0, 2));
  }
  if (!Boolean(windInformation.dampingRationComfort)) {
    windInformation.dampingRationComfort = Number(extractData(lineArray, "舒适度验算用阻尼比", 0, 1));
  }
...
export function extractSeismicInformation(
  ...
  if (!Boolean(seismicInformation.use2015GB18306)) {
    seismicInformation.use2015GB18306 = extractData(lineArray, "按地震动区划图GB18306-2015计算", 0, 1);
  }
  ...
  if (!Boolean(seismicInformation.characteristicPeriod)) {
    seismicInformation.characteristicPeriod = Number(extractData(lineArray, "特征周期", 0, 1));
  }
  if (!Boolean(seismicInformation.dampingRatio)) {
    seismicInformation.dampingRatio = Number(extractData(lineArray, "结构的阻尼比", 0, 1));
  }
  if (!Boolean(seismicInformation.periodReductionFactor)) {
    seismicInformation.periodReductionFactor = Number(extractData(lineArray, "周期折减系数", 0, 1));
  }
  if (!Boolean(seismicInformation.eccentricityX)) {
    seismicInformation.eccentricityX = Number(extractData(lineArray, "X向偶然偏心值", 0, 1));
  }
  if (!Boolean(seismicInformation.eccentricityY)) {
    seismicInformation.eccentricityY = Number(extractData(lineArray, "Y向偶然偏心值", 0, 1));
  }
  if (!Boolean(seismicInformation.maxSpectrumValue)) {
    seismicInformation.maxSpectrumValue = Number(extractData(lineArray, "地震影响系数最大值", 0, 1));
  }
  if (!Boolean(seismicInformation.maxSpectrumValueL3)) {
    seismicInformation.maxSpectrumValueL3 = Number(extractData(lineArray, "罕遇地震影响系数最大值", 0, 1));
  }
  if (!Boolean(seismicInformation.additionalDampingRatio)) {
    seismicInformation.additionalDampingRatio = Number(extractData(lineArray, "最大附加阻尼比", 0, 1));
  }
  if (!Boolean(seismicInformation.modifiedSeismicReductionFactor)) {
    seismicInformation.modifiedSeismicReductionFactor = Number(extractData(lineArray, "调整后的水平向减震系数", 0, 1));
  }
  
  seismicInformation.allExtracted = checkObjectKeysIfAllExtracted(seismicInformation);
...
export function extractConstraintFloorStiffnessRatio(
  ...
  if (!Boolean(constraintFloorStiffnessRatio.storeyID)) {
    constraintFloorStiffnessRatio.storeyID = Number(extractData(lineArray, "地下室层号", 0, 1));//why can not use array methon
    constraintFloorStiffnessRatio.towerID = Number(extractData(lineArray, "地下室层号", 0, 3));//why can not use array methon
  }
  if (!Boolean(constraintFloorStiffnessRatio.stiffnessX0)) {
    constraintFloorStiffnessRatio.stiffnessX0 = Number(extractData(lineArray, "X方向地下一层剪切刚度", 0, 1));//why can not use array methon
    constraintFloorStiffnessRatio.stiffnessX1 = Number(extractData(lineArray, "X方向地下一层剪切刚度", 0, 3));//why can not use array methon
    constraintFloorStiffnessRatio.ratioX = Number(extractData(lineArray, "X方向地下一层剪切刚度", 0, 5));//why can not use array methon
  }
  if (!Boolean(constraintFloorStiffnessRatio.stiffnessY0)) {
    constraintFloorStiffnessRatio.stiffnessY0 = Number(extractData(lineArray, "Y方向地下一层剪切刚度", 0, 1));//why can not use array methon
    constraintFloorStiffnessRatio.stiffnessY1 = Number(extractData(lineArray, "Y方向地下一层剪切刚度", 0, 3));//why can not use array methon
    constraintFloorStiffnessRatio.ratioY = Number(extractData(lineArray, "Y方向地下一层剪切刚度", 0, 5));//why can not use array methon
  }

  constraintFloorStiffnessRatio.allExtracted = checkObjectKeysIfAllExtracted(constraintFloorStiffnessRatio);
...
export function extractOverturningCheck(
  ...
  if (!Boolean(overturningCheck.mrWindX)) {
    ...
    overturningCheck.zeroAreaWindX = Number(extractData(lineArray, "X向风", 0, 4));//why can not use array methon
  }
  if (!Boolean(overturningCheck.mrWindY)) {
    ...
    overturningCheck.zeroAreaWindY = Number(extractData(lineArray, "Y向风", 0, 4));//why can not use array methon
  }
  if (!Boolean(overturningCheck.mrSeismicX)) {
    ...
    overturningCheck.movSeismicX = Number(extractData(lineArray, "X地震", 0, 2));//why can not use array methon
    overturningCheck.ratioSeismicX = Number(extractData(lineArray, "X地震", 0, 3));//why can not use array methon
    overturningCheck.zeroAreaSeismicX = Number(extractData(lineArray, "X地震", 0, 4));//why can not use array methon
  }
  if (!Boolean(overturningCheck.mrSeismicY)) {
    ...
    overturningCheck.movSeismicY = Number(extractData(lineArray, "Y地震", 0, 2));//why can not use array methon
    overturningCheck.ratioSeismicY = Number(extractData(lineArray, "Y地震", 0, 3));//why can not use array methon
    overturningCheck.zeroAreaSeismicY = Number(extractData(lineArray, "Y地震", 0, 4));//why can not use array methon
  }

  overturningCheck.allExtracted = checkObjectKeysIfAllExtracted(overturningCheck);
