    ...
    if (flag === 'keyStable') {
      ...
      if (innerFlag === 'seismic') {
        if (!wmass.stableCheck.allExtracted) {
          wmass.stableCheck = extractStableCheckSeismic(
            lineArray,
            wmass.stableCheck,
          );
        }
        ...
        if (!wmass.stableCheck.allExtracted) {
          wmass.stableCheck = extractStableCheckWind(
            lineArray,
            wmass.stableCheck,
          );
        }
...
export function extractStableCheckSeismic(
  ...
  if (!Boolean(stableCheck.seismicRatioX)) {
    stableCheck.seismicRatioX = Number(
      extractData(lineArray, 'X向刚重比', 0, 2),
    );
  }
  if (!Boolean(stableCheck.seismicRatioY)) {
    stableCheck.seismicRatioY = Number(
      extractData(lineArray, 'Y向刚重比', 0, 2),
    );
  }
...
export function extractWIndComfort(
  ...
  if (!Boolean(windComfort.accelerationAlongX)) {
    windComfort.accelerationAlongX = Number(
      extractData(lineArray, 'X向顺风向顶点最大加速度(m/s2)', 0, 1),
    );
  }
  if (!Boolean(windComfort.accelerationCrossX)) {
    windComfort.accelerationCrossX = Number(
      extractData(lineArray, 'X向横风向顶点最大加速度(m/s2)', 0, 1),
    );
  }
  if (!Boolean(windComfort.accelerationAlongY)) {
    windComfort.accelerationAlongY = Number(
      extractData(lineArray, 'Y向顺风向顶点最大加速度(m/s2)', 0, 1),
    );
  }
  if (!Boolean(windComfort.accelerationCrossY)) {
    windComfort.accelerationCrossY = Number(
      extractData(lineArray, 'Y向横风向顶点最大加速度(m/s2)', 0, 1),
    );
  }
